...
router.post("/", async (req, res) => {
    const { title, genre, isbn, publicationDate, description, available } = req.body;
...
router.put('/:id', async (req, res) => {
    try {
        const { title, genre, isbn, publicationDate, description, available, author } = req.body;
        const book = await Book.findByIdAndUpdate(req.params.id, { title, genre, isbn, publicationDate, description, available, author }, { new: true }).populate('author');
        if (!book) return res.status(404).json({ message: "Book not found" });
        res.json(book);
    } catch (err) {
        res.status(400).json({ message: err.message });
